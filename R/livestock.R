...
# MM.GHG.I = MM.N2O.I * GWP.N2O * 10^-6                                               # Estimates indirect GHG emission from manure management
...
livestock <- function(L.AD, MM.AD, L.EF = IPCC.2006.L.EF, MM.EF = IPCC.2006.MM.EF) {
  ...
  L.MM <- left_join(L, MM.AD, by = "Livestock.Subtype")
  L.MM <- left_join(L.MM, MM.EF, by = c("Livestock.Subtype", "MMS.Subtype"))
  ...
  L.Y.LT <- L.MM %>% mutate(Year = as.factor(Year),
            ...
            summarize(EF.CH4 = sum(EF.CH4),
                      ...
                      N.MA = sum(N.MA),
                      N.M = sum(N.M))
  EF.MM.LT = L.Y.LT %>% select(Year, Livestock.Type, EF.CH4, MM.CH4, MM.N2O.D, MM.N2O.I)
  L.N.LT = L.Y.LT %>% select(Year, Livestock.Type, N.UD, N.M)
  
  # Estimates total enteric fermentation and manure management GHG emissions and N inputs by year
  L.Y <- L.Y.L %>% group_by(Year) %>%
         summarize(EF.CH4 = sum(EF.CH4),
                   MM.CH4 = sum(MM.CH4),
                   MM.N2O.D = sum(MM.N2O.D),
                   MM.N2O.I = sum(MM.N2O.I),
